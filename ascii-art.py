...
def get_font_object(show_warnings=True):
    ...
    if show_warnings is True and len(user_font_file) > 0 and not file_exists(user_font_file):
        print(f'Warning: could not find user-specified font file: "{user_font_file}"; using alternate font: "{font_filename}".')
...
def get_user_config():
    ...
    input_filename = args.input if "input" in args and args.input is not None else ""
    path = prompt_for_image_file() if input_filename == "" else input_filename
    image = Image.open(path)
    new_image_width = int(args.resize_image_width, base=10) if "resize_image_width" in args and args.resize_image_width is not None else image.width
...
def resize(image, new_width):
    if new_width is None: return image
